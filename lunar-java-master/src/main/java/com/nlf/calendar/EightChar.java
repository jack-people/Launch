...
public class EightChar {
  ...
  private String getDiShi(int zhiIndex) {
    int index = LunarUtil.CHANG_SHENG_OFFSET.get(getDayGan()) + (getDayGanIndex() % 2 == 0 ? zhiIndex : -zhiIndex);
    if (index >= 12) {
      index -= 12;
    }
    if (index < 0) {
  ...
  public String getMingGong() {
    int monthZhiIndex = 0;
    int timeZhiIndex = 0;
    LunarUtil.IndexValue iv = LunarUtil.find(lunar.getMonthZhiExact(), LunarUtil.MONTH_ZHI);
    if (null != iv) {
      monthZhiIndex = iv.getIndex();
    }
    iv = LunarUtil.find(lunar.getTimeZhi(), LunarUtil.MONTH_ZHI);
    if (null != iv) {
