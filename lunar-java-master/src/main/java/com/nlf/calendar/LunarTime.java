...
public class LunarTime {
  ...
  public NineStar getNineStar() {
    ...
    boolean asc = solarYmd.compareTo(jieQi.get(I18n.getMessage("jq.dongZhi")).toYmd()) >= 0 && solarYmd.compareTo(jieQi.get(I18n.getMessage("jq.xiaZhi")).toYmd()) < 0;
    int[] offset = asc ? new int[]{0, 3, 6} : new int[]{8, 5, 2};
    int start = offset[lunar.getDayZhiIndex() % 3];
    int index = asc ? (start + zhiIndex) : (start + 9 - zhiIndex);
